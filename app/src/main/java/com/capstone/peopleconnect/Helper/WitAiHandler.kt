...
class WitAiHandler(private val context: Context) {
    ...
    interface WitAiCallback {
        fun onResponse(bookDay: String, startTime: String, endTime: String, rating: String, serviceType: String, intentName: String)
    ...
    private fun parseResponse(response: String, callback: WitAiCallback) {
            ...
            var intentName = "Intent not found"
            if (intentsArray != null && intentsArray.length() > 0) {
                // Get the first intent (if any)
                val intentObject = intentsArray.optJSONObject(0)
                intentName = intentObject?.optString("name", "Intent not found") ?: "Intent not found"
                Log.d("Intent", "Intent: $intentName")
             }
        ...
        (context as? ClientMainActivity)?.runOnUiThread {
            callback.onResponse(bookDay ?: "Unknown", startTime ?: "Unknown", endTime ?: "Unknown", rating, serviceType, intentName)
        }
    }
    ...
    private fun formatDayOfWeek(dateTimeStr: String): String {
        return try {
            ...
            // Return the formatted string with the date and day of the week
            "$datePart , $dayOfWeek"
        } catch (e: Exception) {
